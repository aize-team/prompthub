...
import { useRouter } from 'next/navigation'; // Added for navigation if needed, and for consistency with main page
import { useLanguage } from '@/context/LanguageContext'; // Assuming useLanguage is available for t function
...
function ContributeContent() {
  ...
  const router = useRouter(); // Added for navigation if needed, and for consistency with main page
  const { t } = useLanguage(); // Assuming useLanguage is available for t function

  const [currentDisplayStep, setCurrentDisplayStep] = useState(1);
  const [step1PromptIdea, setStep1PromptIdea] = useState('');
  ...
  useEffect(() => {
    ...
    if (cachedPromptForm && cachedPromptStep === '2') {
      try {
        ...
        setPromptData({
          ...
          tags: formData.tags || '', // Ensure tags are handled as string initially
        ...
        setCurrentDisplayStep(2); // Move to step 2
        dataLoaded = true;
    ...
    if (dataLoaded) return; // Don't proceed if localStorage load was successful
  ...
  useEffect(() => {
    if (currentDisplayStep === 2 && !isLoggedIn && isValid && !notification.show) {
      setNotification({
        show: true,
        type: 'info',
  ...
  const handleStep1Generate = async () => {
    if (!step1PromptIdea.trim()) {
      setStep1Error('Prompt idea cannot be empty.'); // Consider using t() for localization
      return;
    }
    ...
    try {
      const response = await fetch('/api/process-prompt', {
        ...
        body: JSON.stringify({ promptIdea: step1PromptIdea }),
      });

      if (!response.ok) {
        throw new Error('Failed to process prompt idea.'); // Consider using t()
      }
      ...
      setPromptData({
        ...
        model: data.model || '',
        tags: data.tags || '', // API might return array or string, ensure consistency
      ...
      console.error('Error processing prompt idea in contribute:', error);
      setStep1Error(error.message || 'An error occurred while processing.'); // Consider using t()
    } finally {
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-white/95 dark:bg-gray-800/95 rounded-lg p-0.5 backdrop-blur-sm">
                    ...
                    <div className="relative">
                      <textarea
                        value={step1PromptIdea}
                        onChange={(e) => setStep1PromptIdea(e.target.value)}
                      ...
                      <div className="absolute bottom-2 right-3 text-xs text-gray-400 dark:text-gray-500 font-mono">
                        {step1PromptIdea.length} {t('contribute.step1.characters')}
                      </div>
                    </div>
                  </div>
                </div>
  ...
  // Step 2: Full editor (existing layout)
  return (
    <ContributeLayout
